...
location of your Java installation."
    ...
    JAVACMD="java"
    which java >/dev/null 2>&1 || die "JAVA_HOME is not set and 'java' is not on the PATH.
...
# Add the jar to the CLASSPATH and run the application
...
fun DynamicField(property: PropertySchema, value: JsonElement?, onValueChange: (Any?) -> Unit, getRelatedData: (String) -> StateFlow<Resource<List<JsonObject>>>, getRelatedSchema: (String) -> EntitySchema) {
    ...
    when (property.type) {
        ...
        "BOOLEAN" -> Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
        ...
        "RELATIONSHIP" -> {
            ...
            ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = !expanded }) {
                ...
                ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                    ...
                    if (relatedData is Resource.Loading) { DropdownMenuItem(text = { Text("Loading...") }, onClick = {}) }
                }
            }
        }
    }
