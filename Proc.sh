...
echo "### STEP 2: Generating Procedural App Framework (v3 - Dependency Fix) ###"

# --- Check that the environment is set up ---
if [ -z "$ANDROID_SDK_ROOT" ] && [ -z "$ANDROID_HOME" ]; then
    echo "ERROR: ANDROID_SDK_ROOT or ANDROID_HOME is not set." >&2
    echo "Please run 'sudo bash 1_setup_environment.sh' first and then START A NEW SHELL SESSION." >&2
    exit 1
fi
        ...
        for prop in entity['properties']:
            ...
            if prop.get('isPrimaryKey'):
                ...
                if related_entity:
                    ...
                    # Determine FK type based on the related PK's type
                    fk_sql_type = 'UUID' if next((p for p in related_entity['properties'] if p['name'] == related_pk), {}).get('type') == 'UUID' else 'TEXT'
                    cols.append(f'    "{col_name}" {fk_sql_type} REFERENCES public."{related_table}"("{related_pk}") ON DELETE SET NULL')
                else:
                    cols.append(f'    "{col_name}" TEXT') # Fallback if relation is broken
            else:
                 cols.append(f'    "{col_name}" {get_sql_type(prop_type)}')
...
# --- Base64 encoded gradle-wrapper.jar for Gradle 8.6 ---
GRADLE_WRAPPER_JAR_B64=""
...
EOF
echo "$GRADLE_WRAPPER_JAR_B64" | base64 --decode > gradle/wrapper/gradle-wrapper.jar
cat > gradlew << 'EOF'
#!/usr/bin/env bash
PRG="$0"
...
<?xml version="1.0" encoding="utf-8"?><data-extraction-rules><cloud-backup><exclude domain="root" /></cloud-backup></data-extraction-rules>
EOF
...
class DynamicRepository @Inject constructor(private val client: SupabaseClient) {
    private fun <T> apiCall(call: suspend () -> T): Flow<Resource<T>> = flow {
        ...
        if (client.auth.currentUserOrNull() == null) {
            try { client.auth.signInWith(io.github.jan.supabase.gotrue.providers.builtin.Email) { email = "test@example.com"; password = "password123"} } catch (e: Exception) {
                try { client.auth.signUpWithEmail("test@example.com", "password123") } catch (e2: Exception) { /* User likely exists, ignore */ }
            }
        }
    ...
    fun saveEntity(table: String, data: Map<String, JsonElement>, pkColumn: String, id: String?): Flow<Resource<Unit>> = apiCall {
        if (id != null && id.isNotBlank()) { client.postgrest.from(table).update(data) { filter { eq(pkColumn, id) } } } else { client.postgrest.from(table).insert(data) }; Unit
    }
...
private val DarkColorScheme = darkColorScheme(primary = Color(0xFFBB86FC), secondary = Color(0xFF03DAC5))
private val LightColorScheme = lightColorScheme(primary = Color(0xFF6200EE), secondary = Color(0xFF03DAC5))
...
@Composable fun AppNavigation() {
    ...
    NavHost(navController = navController, startDestination = "dashboard") {
        ...
        composable("form/{entityName}?id={entityId}", arguments = listOf(
...
@Composable fun ConfirmationDialog(show: Boolean, onDismiss: () -> Unit, onConfirm: () -> Unit, title: String, text: String) {
    if (show) { AlertDialog(onDismissRequest = onDismiss, title = { Text(title) }, text = { Text(text) }, confirmButton = { TextButton(onClick = { onConfirm(); onDismiss() }) { Text("Confirm") } }, dismissButton = { TextButton(onClick = onDismiss) { Text("Cancel") } }) }
}
EOF
...
@Composable fun EntityListScreen(navController: NavController, entityName: String, viewModel: EntityListViewModel = hiltViewModel()) {
    ...
    ConfirmationDialog(show = showDeleteDialog != null, onDismiss = { showDeleteDialog = null }, onConfirm = { showDeleteDialog?.let { viewModel.deleteItem(it) } }, title = "Delete Item?", text = "Are you sure?")
    Scaffold(
                ...
                is Resource.Success -> {
                    LazyColumn(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        item {
                            ...
                            Button(onClick = { viewModel.save() }, enabled = saveState !is Resource.Loading, modifier = Modifier.fillMaxWidth()) { Text("Save") }
                            if(saveState is Resource.Loading) CircularProgressIndicator(modifier = Modifier.padding(top = 8.dp))
                            if(saveState is Resource.Error) Text(saveState.message ?: "Error saving", color=MaterialTheme.colorScheme.error, modifier = Modifier.padding(top = 8.dp))
                        }
                    }
                }
...
class EntityFormViewModel @Inject constructor(private val repo: DynamicRepository, private val schema: AppSchema, ssh: SavedStateHandle) : ViewModel() {
    ...
    fun save() = viewModelScope.launch {
        ...
        repo.saveEntity(entitySchema.tableName, currentData, entitySchema.primaryKey, entityId).collect { _saveState.value = it }
    }
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun DynamicField(property: PropertySchema, value: JsonElement?, onValueChange: (Any?) -> Unit, getRelatedData: (String) -> StateFlow<Resource<List<JsonObject>>>, getRelatedSchema: (String) -> EntitySchema) {
    val stringValue = if (value is JsonNull || value == null) "" else value.jsonPrimitive.content
    when (property.type) {
        "TEXT", "NUMBER" -> OutlinedTextField(value = stringValue, onValueChange = onValueChange, label = { Text(property.label) }, singleLine = true, modifier = Modifier.fillMaxWidth())
        "TEXT_AREA" -> OutlinedTextField(value = stringValue, onValueChange = onValueChange, label = { Text(property.label) }, minLines = 3, modifier = Modifier.fillMaxWidth())
        "BOOLEAN" -> Row(verticalAlignment = Alignment.CenterVertically) {
            Text(property.label, modifier = Modifier.weight(1f)); Switch(checked = value?.jsonPrimitive?.booleanOrNull ?: false, onCheckedChange = onValueChange)
        }
        "RELATIONSHIP" -> {
            ...
            val selectedItemText = items.find { it[relatedSchema.primaryKey]?.jsonPrimitive?.content == stringValue }?.formatDisplayProperty(relatedSchema) ?: "Select ${property.label}..."
            ExposedDropdownMenuBox(expanded = expanded, onExpandedChange = { expanded = !expanded }) {
                OutlinedTextField(value = selectedItemText, onValueChange = {}, readOnly = true, label = { Text(property.label) }, modifier = Modifier.fillMaxWidth().menuAnchor(), trailingIcon = { ExposedDropdownMenuDefaults.TrailingIcon(expanded = expanded) })
                ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                    ...
                    if (relatedData is Resource.Loading) { DropdownMenuItem(text = { Text("Loading...") }, onClick = {}, enabled=false) }
                }
            }
        }
    }
...
fun JsonObject.formatDisplayProperty(schema: EntitySchema?): String {
    ...
    val displayProp = schema.properties.find { it.isDisplay } ?: schema.properties.firstOrNull { it.name == schema.displayProperty } ?: schema.properties.first()
